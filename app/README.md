...
Browser app for the `test_zebec_payroll_v3.aleo` payroll program (testnet). It is the
...
- `vite.config.ts` sets `build.target: "esnext"` (the SDK's wasm init uses
  ...
  `build/test_zebec_payroll_v3/test_zebec_payroll_v3.aleo?raw` deploy artifact.
