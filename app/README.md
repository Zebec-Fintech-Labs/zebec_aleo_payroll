...
Browser app for the `test_zebec_payroll_v2.aleo` payroll program (testnet). It is the
...
- `vite.config.ts` sets `build.target: "esnext"` (the SDK's wasm init uses
  ...
  `build/test_zebec_payroll_v2/test_zebec_payroll_v2.aleo?raw` deploy artifact.
