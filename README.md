...
Compiles `src/main.leo` per `program.json` into `build/test_zebec_stream_v3/` (ABI, compiled `.aleo`, interfaces).

### Deploy / upgrade

```bash
yarn run:deploy    # scripts/deploy.ts — deploys build/test_zebec_stream_v3/test_zebec_stream_v3.aleo
...
Program id: **`test_zebec_stream_v3.aleo`** (Leo `4.4.1`, depends only on `credits.aleo`).
